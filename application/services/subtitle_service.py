...
class SubtitleService:
    ...
    def process_subtitles(
        ...
        if translate:
            subtitle_blocks = self._translate_blocks(
                subtitle_blocks, target_language, progress_callback
            )
    ...
    def _extract_blocks(
        self, content: str, progress_callback: Callable
    ) -> List[List[str]]:
        """
    ...
    def _translate_blocks(
        self, blocks: List[List[str]], target_language: str, progress_callback: Callable
    ) -> List[List[str]]:
        """
        ...
        progress_callback(
            "status", f"Translating {total_blocks} subtitles (parallel)..."
        )
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            future_to_index = {
                executor.submit(translate_single_block, i, b): i
    ...
    def _optimize_blocks(
        self, blocks: List[List[str]], progress_callback: Callable
    ) -> List[List[str]]:
        """
        ...
        for i, block in enumerate(blocks):
            ...
            actual_matches = [
                m.group(0) for m in timestamp_pattern.finditer(timestamp_line)
            ]
            ...
            if not end_time:
                if i + 1 < len(blocks):
                    ...
                    next_matches = [
                        m.group(0)
                        for m in timestamp_pattern.finditer(next_timestamp_line)
                    ]
                    if next_matches:
