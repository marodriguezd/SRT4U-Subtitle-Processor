...
class VideoBurnerService:
    ...
    def get_ffmpeg_path() -> Optional[str]:
        """
        Localiza el binario de FFmpeg en el sistema o dentro del paquete de la app.
        Prioridad:
        1. Directorio temporal de PyInstaller (sys._MEIPASS)
        2. Directorio bin/ local de la aplicación
        3. PATH global del sistema operativo
        """
        # 1. PyInstaller bundled path
        if hasattr(sys, "_MEIPASS"):
            exe_name = "ffmpeg.exe" if sys.platform == "win32" else "ffmpeg"
            p1 = os.path.join(sys._MEIPASS, exe_name)
            if os.path.isfile(p1) and os.access(p1, os.X_OK):
                return p1
            p2 = os.path.join(sys._MEIPASS, "bin", exe_name)
        ...
        local_bin = os.path.join(app_root, "bin", local_exe)
...
Style: Default,Arial,{fontsize},{primary_color},&H000000FF,&H00000000,{back_color},-1,0,0,0,100,100,0,0,{border_style},{outline},{shadow},2,20,20,22,1
...
class BurnInWorker(QThread):
    ...
    def run(self):
        ...
        if not ffmpeg_bin:
            self.failed.emit(
                "No se encontró el ejecutable FFmpeg en el sistema ni en el paquete de la aplicación.\n"
